classdef Environment_features_6_minimax < rl.env.MATLABEnvironment
    ...
    %% Necessary Methods
    methods              
        function this = Environment_features_6_minimax()
            % This method sets the observation information and action
            % information of the environment.

            % Initialize Observation settings
            ObservationInfo = rlNumericSpec([49, 1]);
...
%          STEPS START WITH RL
        function [Observation,Reward,IsDone,LoggedSignals] = step(this,Action) 
            ...
            % Unpack state vector
            Board = reshape(this.State(1:42), 6,7);
            ...
            additional_row = transform_matrix_features_env6(Board);
            observation = reshape(Board, 42, 1);

            % Update system states
            this.State = [observation; additional_row'];
            Observation=[observation; additional_row'];
        ...
        function InitialObservation = reset(this)
            ...
            % Flatten the Board matrix into a 1-dimensional array (State vector)
            Observation = reshape(Board, 49,1);
    ...
    methods               
        ...
        function set.State(this,state)
            validateattributes(state,{'numeric'},{'finite','real','vector','numel',49},'','State');
